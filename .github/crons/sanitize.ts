...
export async function parseSanitizedHTML(
  html: string
): Promise<SanitizedHTMLObject[]> {
...
export function filterSanitizedHTML(
  sanitized: SanitizedHTMLObject[],
  filters: string[]
): { [key: string]: string }[] {
...
export async function scrapeArticle(
  ...
  if (exist.newsbg_by_pk) {
    console.log("exist");
    return;
  }
  await db.insert({ nid: url });
  ...
  const docx = {
    title: article?.title
      .replace("- Mediapool.bg", "")
      .replace("- новини СЕГА", "") as string,
